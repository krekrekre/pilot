  ...
  return (
    <div className="flex-1 max-w-5xl mx-auto w-full px-5 py-12">
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        {GAMES.map(g => {
          const cardClass = `group rounded-2xl border p-6 flex gap-5 transition ${g.available ? 'border-slate-200 hover:border-[#8c1d68]/40 hover:shadow-md cursor-pointer bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8c1d68]/40' : 'border-slate-100 bg-slate-50/60 opacity-60'}`;

          const content = (
            <>
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl flex-shrink-0 ${g.available ? 'bg-[#8c1d68]/8' : 'bg-slate-100'}`}>
                {g.icon}
              </div>
              <div className="flex flex-col gap-1.5 flex-1">
                <div className="flex items-center gap-2">
                  <h2 className="font-bold text-slate-900 text-sm">{g.title}</h2>
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${g.available ? 'bg-[#8c1d68]/10 text-[#8c1d68]' : 'bg-slate-200 text-slate-400'}`}>
                    {g.available ? g.category : 'Coming soon'}
                  </span>
                </div>
                <p className="text-xs text-slate-500 leading-relaxed">{g.desc}</p>
                {g.available && (
                  <span className="mt-2 text-xs font-bold text-[#8c1d68] group-hover:underline w-fit">
                    Start session →
                  </span>
                )}
              </div>
          ...
          return g.available ? (
            <Link key={g.slug} href={`/games/${g.slug}`} className={cardClass}>
              {content}
            </Link>
          ) : (
